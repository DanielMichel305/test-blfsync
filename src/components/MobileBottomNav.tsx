...
export default function MobileBottomNav({ currentUser, activeTab, setActiveTab, dashboardSubTab, setDashboardSubTab, onNavigate }: { currentUser: Donor | null; activeTab: AppTab; setActiveTab: (tab: AppTab) => void; dashboardSubTab: DashboardTab; setDashboardSubTab: (tab: DashboardTab) => void; onNavigate?: (path: string) => void }) {
  const { t } = useLanguage();
  if (!currentUser) return null;
  const openDashboard = (subTab: DashboardTab) => {
    const paths: Record<DashboardTab, string> = { overview: '/', dashboard: '/dashboard', prayer: '/prayer', profile: '/profile', referrals: '/referrals' };
    if (onNavigate) onNavigate(paths[subTab]);
    else { setActiveTab('dashboard'); setDashboardSubTab(subTab); }
  };
